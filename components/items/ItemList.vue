<template>
  <div class="mb-8">
    <div class="flex justify-between items-center mb-3">
      ...
      <div v-if="totalPrice > 0" class="text-sm font-medium bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">
    ...
    <div v-if="items.length > 0">
      <div v-for="category in groupedCategories" :key="category.id" class="mb-4">
        <h4 class="text-md font-semibold text-gray-600 dark:text-gray-300 px-3 py-1 bg-gray-50 dark:bg-gray-700 rounded-t-lg flex justify-between items-center">
          <span>{{ category.name }}</span>
          <span v-if="getCategoryTotal(category.id) > 0" class="text-sm text-gray-500 dark:text-gray-400">
            {{ formatPrice(getCategoryTotal(category.id)) }}
          </span>
        </h4>
        <ul class="bg-white dark:bg-gray-800 rounded-b-lg shadow-sm overflow-hidden divide-y divide-gray-100 dark:divide-gray-700">
          <ItemListItem 
