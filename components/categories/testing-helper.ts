...
export function diagnoseCategories(): ((categoryId: string, newName: string) => boolean) | undefined {
  try {
    ...
    if (parsedCategories.activeTemplateId) {
      if (parsedCategories.customTemplates && parsedCategories.customTemplates[parsedCategories.activeTemplateId]) {
    ...
    return function forceUpdateCategory(categoryId: string, newName: string): boolean {
      ...
      const newLists = parsedLists.map(list => {
        if (Array.isArray(list.items)) {
          list.items = list.items.map(item => {
            if (item.category && typeof item.category === 'object' && item.category.id === categoryId) {
              updatedCount++;
              return {
                ...item,
                category: {
      ...
      if (updatedCount > 0) {
        ...
        console.log(`[Diagnose] ${updatedCount} Artikel aktualisiert mit neuer Kategorie "${newName}"`);
        return true;
      } else {
