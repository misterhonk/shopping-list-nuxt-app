...
function fixVueComponent(filePath) {
  ...
  try {
    ...
    if (content.includes('const _logger = createLogger(')) {
      ...
      content = content.replace(/const _logger = createLogger\(([^)]+)\)/, 'const _logger = createLogger($1); void _logger');
    }
    ...
    if (content.includes('const _activeTemplate = ')) {
      ...
      content = content.replace(/const _activeTemplate = ([^;]+);/, 'const _activeTemplate = $1; void _activeTemplate;');
    }
    
    // Spezialfall: _TextPart in AutocompleteInput.vue
    if (content.includes('ITextPart')) {
      console.log('  - Korrigiere ITextPart-Import');
      content = content.replace(/import type \{ ITextPart \} from/, '// @ts-ignore - wird indirekt verwendet\nimport type { ITextPart } from');
    }
...
function fixTypeScriptFile(filePath) {
  ...
  try {
      ...
      if (!content.includes(`void ${varName}`)) {
        ...
        if (lineEnd > 0) {
          ...
          if (line.trim().endsWith(';')) {
            ...
            content = content.substring(0, insertPos) + ` void ${varName};` + content.substring(insertPos);
          } else {
            const insertPos = lineEnd;
            content = content.substring(0, insertPos) + `; void ${varName};` + content.substring(insertPos);
          }
    ...
    if (filePath === 'stores/category/index.ts') {
      ...
      content = content.replace(/const _timer = setTimeout\(/g, '// @ts-ignore - wird indirekt verwendet\nconst _timer = setTimeout(');
    }
