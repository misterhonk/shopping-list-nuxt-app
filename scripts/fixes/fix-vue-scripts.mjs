...
async function fixVueScripts() {
  ...
  for (const filePath of vueFiles) {
    ...
    if (content.includes('<script setup>') && !content.includes('<script setup lang="ts">')) {
      ...
      const hasTypeAnnotations = /:(\s*)[A-Z]|:\s*void|:\s*string|:\s*number|:\s*boolean|:\s*any|<[A-Z][a-zA-Z]*>/g.test(content);
      
      if (hasTypeAnnotations) {
        const updatedContent = content.replace('<script setup>', '<script setup lang="ts">');
        fs.writeFileSync(filePath, updatedContent, 'utf8');
        console.log(`✓ Updated script setup in ${path.relative(projectRoot, filePath)} to use TypeScript`);
        fixedCount++;
      }
    }
  }
