...
async function fixArrowFunctions() {
  ...
  for (const relativeFilePath of PROBLEMATIC_FILES) {
    ...
    const updatedContent1 = content.replace(arrowParamReturnTypeRegex, (match, params, returnType) => {
    ...
    const updatedContent2 = content.replace(callbackTypeRegex, (match, name, params, returnType, callback) => {
    ...
    const functionReturnArrowRegex = /function\s*\(([^)]*)\)\s*:\s*void\s*=>\s*(\w+(?:<[^>]+>)?(?:\[\])?)/g;
    const updatedContent3 = content.replace(functionReturnArrowRegex, (match, params, returnType) => {
