...
function findInterfaceNamingIssues() {
  try {
    ...
    try {
      const grepCmd = "npx eslint --ext .ts,.vue . | grep -E 'Interface name.*must have one of the following prefixes: I'";
  ...
  for (const filePath in fileInterfaceMap) {
    try {
      ...
      for (const interfaceName of interfaceNames) {
        ...
        const importExportPattern = new RegExp(`([import|export]\\s+{[^}]*?)\\b${interfaceName}\\b`, 'g');
