...
const files = ['components/items/ItemList.vue', 'composables/shoppingItems/useItemForm.ts'];
...
function fixRefAsOperand(filePath) {
  ...
  try {
    ...
    const computedMatches = [
      ...content.matchAll(/([^\.a-zA-Z0-9_])(_[a-zA-Z0-9_]+)(?!\.value)(?![a-zA-Z0-9_])/g),
    ];
    ...
    for (let i = computedMatches.length - 1; i >= 0; i--) {
      ...
      if (refName.startsWith('_') && !match[0].includes('void') && !match[0].includes('=')) {
        ...
        // Füge .value hinzu
        newContent =
          newContent.substring(0, startIndex) + refName + '.value' + newContent.substring(endIndex);
