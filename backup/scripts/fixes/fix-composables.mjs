...
async function fixComposables() {
  ...
  const specificFiles = [path.join(projectRoot, 'composables/useDarkMode.ts')];
  ...
  const utilsFiles = fs.existsSync(utilsDir) ? scanDir(utilsDir, /\.ts$/) : [];
...
function fixComposableReturnTypes(content) {
  ...
  content = content.replace(/(return\s*\{[^}]*\})\s*(?!\s*as)/g, '$1 as const');
