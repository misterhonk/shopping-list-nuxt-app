...
fixedContent = fixedContent.replace(
  /localStorage\.setItem\(LAST_CHECK_KEY, _now\.toString\(\)\);/g,
  'localStorage.setItem(LAST_CHECK_KEY, now.toString());'
);

// 3. Fix registration variable naming
fixedContent = fixedContent.replace(
  /navigator\.serviceWorker\.getRegistration\(\)\.then\(_reg => {/g,
  'navigator.serviceWorker.getRegistration().then(reg => {'
);
fixedContent = fixedContent.replace(/if \(_reg\) {/g, 'if (reg) {');
fixedContent = fixedContent.replace(
  /_reg\.update\(\)\.catch\(_err => console\.error\(_err\)\);/g,
  'reg.update().catch(err => console.error(err));'
);

// 4. Fix more registration variables
fixedContent = fixedContent.replace(
  /navigator\.serviceWorker\.getRegistration\(\)\.then\(_registration => {/g,
  'navigator.serviceWorker.getRegistration().then(registration => {'
);
fixedContent = fixedContent.replace(/if \(_registration\) {/g, 'if (registration) {');
fixedContent = fixedContent.replace(
  /__registration\.update\(\)\.catch\(_err => {/g,
  'registration.update().catch(err => {'
);
fixedContent = fixedContent.replace(
  /console\.error\('Fehler beim Update des Service Workers:', _err\);/g,
...
fixedContent = fixedContent.replace(
  /if \(_registration\.waiting\) {/g,
  'if (registration.waiting) {'
);
fixedContent = fixedContent.replace(
  /_registration\.waiting\.postMessage/g,
  'registration.waiting.postMessage'
);
fixedContent = fixedContent.replace(
  /_registration\.unregister\(\)\.then\(\(\) => {/g,
...
fixedContent = fixedContent.replace(
  /console\.error\('Fehler beim Aktualisieren des Service Workers:', _error\);/g,
  "console.error('Fehler beim Aktualisieren des Service Workers:', error);"
);
fixedContent = fixedContent.replace(/} catch \(_error\) {/g, '} catch (error) {');
fixedContent = fixedContent.replace(
  /console\.error\('Fehler beim Löschen der Caches:', _error\);/g,
