...
async function fixParsingErrors() {
  ...
  const filesToProcess = allFiles.filter(
    file => !knownProblematicFiles.some(problematicFile => file.endsWith(problematicFile))
  );
...
async function fixFileParsingErrors(filePath, projectRoot) {
  ...
  const doubleReturnTypeRegex =
    /function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\(([^)]*)\)\s*:\s*void\s*:\s*([a-zA-Z_$][a-zA-Z0-9_$<>[\]]*)/g;
  ...
  const multipleReturnTypesRegex =
    /([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\(([^)]*)\)\s*:\s*([a-zA-Z_$][a-zA-Z0-9_$<>[\]]*)\s*,\s*([a-zA-Z_$][a-zA-Z0-9_$<>[\]]*)/g;
