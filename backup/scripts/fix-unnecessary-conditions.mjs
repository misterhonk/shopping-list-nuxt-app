  ...
  for (const line of lines) {
    ...
    if (currentFile && line.includes('@typescript-eslint/no-unnecessary-condition')) {
      ...
      const match = line.match(
        /(\d+):(\d+)\s+warning\s+(.*?)(@typescript-eslint\/no-unnecessary-condition)/
      );
      if (match) {
...
function processFile(filePath, conditionsToFix) {
  ...
  for (const condition of conditionsToFix) {
    ...
    if (
      ...
      if (nullishIndex >= 0) {
        lines[lineIndex] =
          line.substring(0, nullishIndex) + '||' + line.substring(nullishIndex + 2);
        changes++;
        console.log(`  Linie ${condition.lineNumber}: ?? durch || ersetzt`);
      }
    } else if (
      condition.message.includes('value is always truthy') ||
      ...
      console.log(
        `  Linie ${condition.lineNumber}: Komplexe Bedingung gefunden, manuelle Überprüfung erforderlich`
      );
