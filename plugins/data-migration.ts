...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  try {
    ...
    if (storedLists) {
      ...
      const needsMigration = Array.isArray(lists) && lists.some(list => {
        // Kriterien für Migrationsbedarf
        return (
          !list.hasOwnProperty('createdAt') || 
          !list.hasOwnProperty('modifiedAt') ||
          (Array.isArray(list.items) && list.items.some(item => 
            typeof item.category === 'string' || !item.hasOwnProperty('addedAt')
          ))
        );
      });
      ...
      if (needsMigration) {
        ...
        console.log('[Data Migration] Keine Migration notwendig, Daten bereits im aktuellen Format');
      }
    } else {
