<template>
  <div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
      <h2 class="text-xl font-bold mb-4 text-gray-800 dark:text-white">Aktuelle Liste: {{ currentList.name }}</h2>
      
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Gesamtbetrag</h3>
          <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ formatCurrency(totalAmount) }}</p>
        </div>
        
        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Durchschnitt pro Artikel</h3>
          <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ formatCurrency(averageItemPrice) }}</p>
        </div>
        
        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Teuerster Artikel</h3>
          <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ mostExpensiveItem?.name || '-' }}</p>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ mostExpensiveItem ? formatCurrency(mostExpensiveItem.price) : '-' }}</p>
        </div>
        
        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Anzahl Artikel</h3>
          <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ itemsWithPrice }} / {{ totalItems }}</p>
      ...
      <div v-if="categoryExpenses.length > 0" class="relative pt-1 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
          ...
          >
            ...
            <span class="text-xs text-gray-600 dark:text-gray-300">{{ category.name }}: {{ formatCurrency(category.amount) }} ({{ category.percentage.toFixed(1) }}%)</span>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
      ...
      <p class="text-gray-500 dark:text-gray-400 mb-2">Zusammenfassung Ihrer letzten Einkaufslisten</p>
      
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Liste</th>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Artikel</th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Gesamtbetrag</th>
            </tr>
          </thead>
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="shoppingHistory.length === 0">
              <td colspan="4" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                Keine Einkaufshistorie verfügbar. Alle Ihre abgeschlossenen Einkaufslisten werden hier angezeigt.
              </td>
            </tr>
            <tr v-for="history in shoppingHistory" :key="history.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ history.name }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-500 dark:text-gray-400">{{ formatDate(history.date) }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-500 dark:text-gray-400">{{ history.itemCount }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900 dark:text-white">{{ formatCurrency(history.totalAmount) }}</div>
              </td>
            </tr>
          </tbody>
        </table>
...
const categoryExpenses = computed(() => {
  ...
  allItems.value.forEach(item => {
    ...
    const categoryName = typeof item.category === 'object' ? item.category.name : (item.category || 'Sonstiges');
...
const getCategoryColor = (categoryId) => {
  ...
  const colorClasses = [
    'bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-red-500', 'bg-indigo-500',
    'bg-purple-500', 'bg-pink-500', 'bg-gray-500', 'bg-orange-500', 'bg-teal-500'
  ];
